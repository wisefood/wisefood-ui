  ...
  <div class="page page-center">
    <div class="container container-tight py-4">
      ...
      <div class="card card-md">
        <div class="card-body text-center py-1 p-sm-5 mt-">
          <svg class="img mb-n2" xmlns="http://www.w3.org/2000/svg" height="100" fill="none" viewBox="0 0 800 600">
          ...
          <h1 class="mt-2">Welcome to WiseFood!</h1>
          <p class="text-secondary">
            WiseFood is an AI-powered suite of applications designed to empower citizens to adopt healthier and eco-friendly food choices
          </p>
        </div>

        <div class="hr-text hr-text-center hr-text-spaceless py-0">your profile</div>

        <div class="card-body mt-0">
          <!-- Household name -->
          <div class="mb-2">
            <label class="form-label">Create your Household</label>
            <div class="input-group input-group-flat">
              <input
                ...
                autocomplete="off"
                v-model="householdNameModel"
              />
            </div>
            <div class="form-hint small">
              Choose a name for your household, e.g. <strong>"The Smiths"</strong>
            </div>
          </div>
          ...
          <div class="mb-2">
            ...
            <div class="row align-items-center">
              <div class="col-lg-10">
                <input
                  type="range"
                  class="form-range col-10 custom-range"
                  v-model="sliderModel"
                  min="0"
                  max="10"
                  step="1"
                  :style="{
                    background: `linear-gradient(to right, var(--tblr-primary) 0%, var(--tblr-primary) ${
                      sliderProgress
                    }%, var(--tblr-border-color) ${sliderProgress}%, var(--tblr-border-color) 100%)`
                  }"
                />
              </div>
              <div class="col-lg-2 text-center h2 mt-1 p-0">
                <span id="slider-value">{{ sliderModel }}</span>
              </div>
            </div>
            <div class="form-hint small">
              You will be able to create profiles for each member later
            </div>
          </div>
          ...
          <div class="mb-1">
            ...
            <div class="input-group input-group-flat">
              <select class="form-select" v-model="countryModel">
            ...
            <div class="form-hint small">
              This will be used to ensure your recommendations are relevant to your region
            </div>
          </div>
      ...
      <div v-if="submissionError || creationErrorMessage" class="alert alert-danger mt-3" role="alert">
        {{ submissionError || creationErrorMessage }}
      </div>
      ...
      <div class="row align-items-center mt-3">
        ...
        <div class="col">
          <div class="btn-list justify-content-end">
            <button
              type="button"
              class="btn btn-primary btn-2 d-inline-flex align-items-center"
              :disabled="!canContinue"
              @click="handleContinue"
            >
              <span
                v-if="wizardStore.creatingHousehold"
...
async function checkExistingHousehold() {
  ...
  try {
    ...
    if (existing?.id) {
      wizardStore.setHouseholdId(existing.id);
      wizardStore.setMemberCount(existing.member_count || 1);
      wizardStore.bootstrap({
        householdName: existing.name,
        countryCode: existing.region,
      });
      await router.replace({ name: "dashboard" });
      return;
    }
  } catch (error) {
    // Ignore 404s (no household yet), log others for debugging
    console.debug("No existing household found or fetch failed:", error);
  }

  wizardStore.setCurrentStep(1);
}

const householdNameModel = computed({
  get: () => wizardStore.householdName,
  set: (value: string) => wizardStore.setHouseholdName(value ?? ""),
});

const sliderModel = computed({
  get: () => wizardStore.memberCount,
  set: (value: unknown) => {
    const numeric = typeof value === "number" ? value : Number(value);
    wizardStore.setMemberCount(
      Number.isFinite(numeric) ? Math.min(SLIDER_MAX, numeric) : 1
    );
  },
});

const sliderProgress = computed(() => {
  if (!SLIDER_MAX) return 0;
  return Math.min(100, (wizardStore.memberCount / SLIDER_MAX) * 100);
});

const countryModel = computed({
  get: () => wizardStore.countryCode ?? "",
  set: (value: string) => wizardStore.setCountryCode(value || null),
});

const progressPercent = computed(() => wizardStore.progressPercent);

const canContinue = computed(() => wizardStore.isReadyForCreation && !wizardStore.creatingHousehold);

const creationErrorMessage = computed(() => {
  const err = wizardStore.creationError;
  if (!err) return null;
...
const continueButtonText = computed(() => (wizardStore.creatingHousehold ? "Creating..." : "Continue"));

async function handleContinue() {
  submissionError.value = null;

  try {
    await wizardStore.ensureHouseholdExists();
    wizardStore.advanceStep();
    await router.push({ name: "profile-members" });
  } catch (error) {
    if (error instanceof ApiError) {
      ...
      submissionError.value = "Something went wrong while creating your household.";
    }
  }
}
...
definePage({
  ...
  meta: {
    layout: "intermediate",
    titleKey: "pages.profiles.title",
    descriptionKey: "pages.profiles.description",
  },
});
</script>
<style>
