<template>
  <v-card class="pa-4" flat>
    ...
    <v-btn color="primary" class="ma-2">{{ t("application.filters") }}</v-btn>
    <v-divider></v-divider>
    ...
    <v-data-table
      ...
      :items-per-page-text="t('application.table.itemsPerPage')"
      ...
      <template v-slot:no-data>
        <div class="d-flex flex-column justify-center align-center pa-8">
          ...
          <h3 class="text-h6 mb-2">
            {{ t("application.admin.noApplicationsFound") }}
          </h3>
          <p class="text-body-2 text-medium-emphasis mb-4">
            {{ t("application.admin.noApplicationsDescription") }}
          </p>
        </div>
      </template>
...
const { getApplications } = applicationManagementService();

// API call function
const fetchApplications = async (page = 1, perPage = 10) => {
  loading.value = true;
  try {
    const data = await getApplications(page - 1, perPage);
...
const getTranslatedApplicationType = (type) => {
  const typeTranslations = {
    ...
    JOB_APPLICATION: t("application.types.jobApplication"),
    INTERNSHIP: t("application.types.internship"),
    VOLUNTEER: t("application.types.volunteer"),
    SCHOLARSHIP: t("application.types.scholarship"),
  };
