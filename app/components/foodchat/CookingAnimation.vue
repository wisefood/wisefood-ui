<template>
  ...
  >
    ...
    <div class="relative mb-8">
      ...
      <div class="absolute inset-0 -inset-x-8 -inset-y-4 bg-gradient-radial from-brandp-100/40 via-transparent to-transparent dark:from-brandp-900/20 rounded-full blur-2xl" />
      ...
      <div
        ...
        :style="vesselStyle"
      >
        <svg width="140" height="110" viewBox="0 0 140 110" fill="none" class="drop-shadow-sm">
          ...
          <rect x="115" y="47" width="30" height="6" rx="3" class="fill-brandp-200 dark:fill-brandp-300/60" />
          <rect x="118" y="48" width="24" height="4" rx="2" class="fill-brandp-300/80 dark:fill-brandp-400/40" />
          ...
          <g class="cooking-content">
            <g v-if="currentAction === 'stirring'" class="stirring-spoon">
              ...
              <rect x="67" y="18" width="6" height="32" rx="3" class="fill-brandp-200 dark:fill-brandp-300/70" />
              <ellipse cx="70" cy="18" rx="4" ry="2" class="fill-brandp-300/80 dark:fill-brandp-400/50" />
            </g>
            <g v-if="currentAction === 'frying'" class="frying-bubbles">
              <circle v-for="b in 5" :key="`bubble-${b}`" :cx="40 + b * 12" :cy="50" :r="2 + (b % 3)" class="frying-bubble fill-amber-100/80 dark:fill-amber-200/40" :style="{ animationDelay: `${b * 0.25}s` }" />
            </g>
            ...
            <g v-if="currentAction === 'boiling'" class="boiling-action">
              <circle v-for="b in 5" :key="`boil-${b}`" :cx="45 + b * 10" :cy="52" :r="2 + (b % 2)" class="boiling-bubble fill-sky-100/80 dark:fill-sky-200/40" :style="{ animationDelay: `${b * 0.2}s` }" />
            </g>
            <g v-if="currentAction === 'sauteing'" class="sauteing-action">
              <rect v-for="i in 4" :key="`saute-${i}`" :x="50 + i * 10" y="47" width="5" height="5" rx="1" class="saute-piece" :class="['fill-peach', 'fill-sage', 'fill-lavender', 'fill-cream'][i - 1]" :style="{ animationDelay: `${i * 0.12}s` }" />
            </g>
            <g v-if="currentAction === 'simmering'" class="simmering-action">
              <circle v-for="s in 4" :key="`simmer-${s}`" :cx="48 + s * 12" :cy="52" r="2.5" class="simmer-bubble fill-amber-100/60 dark:fill-amber-200/30" :style="{ animationDelay: `${s * 0.35}s` }" />
            </g>
          </g>
        </svg>
      </div>
      ...
      <div class="absolute inset-0 pointer-events-none">
        <svg
          ...
          :style="{ left: `${ingredient.x}px`, top: `${ingredient.y}px`, animationDelay: `${idx * 0.6}s` }"
    ...
    <div class="text-center space-y-3">
      ...
      <Transition name="fade-desc" mode="out-in">
        <p :key="currentAction" class="text-sm font-light tracking-wide text-stone-400 dark:text-stone-500">
          {{ actionDescription }}
        </p>
      </Transition>
    </div>
    ...
    <div class="mt-8 flex gap-2">
      <div
        ...
        :class="idx === currentActionIndex
          ? 'bg-brandp-400 dark:bg-brandp-500/70 scale-150'
...
<style scoped>
.fill-peach { fill: #FFDAB9; }
.fill-sage { fill: #B2D8B2; }
.fill-lavender { fill: #E6E6FA; }
.fill-cream { fill: #FFFDD0; }

:root.dark .fill-peach { fill: rgba(255, 218, 185, 0.6); }
:root.dark .fill-sage { fill: rgba(178, 216, 178, 0.6); }
:root.dark .fill-lavender { fill: rgba(230, 230, 250, 0.5); }
:root.dark .fill-cream { fill: rgba(255, 253, 208, 0.5); }

.fill-brandp-200 { fill: var(--color-brandp-200); }
.fill-brandp-300 { fill: var(--color-brandp-300); }

:root.dark .fill-brandp-300\/60 { fill: color-mix(in srgb, var(--color-brandp-300) 60%, transparent); }
:root.dark .fill-brandp-400\/40 { fill: color-mix(in srgb, var(--color-brandp-400) 40%, transparent); }

.steam-particle { animation: steam-rise 3s ease-out infinite; opacity: 0; }
@keyframes steam-rise {
  0% { transform: translateY(0) scale(0.7); opacity: 0; }
  15% { opacity: 0.5; }
  100% { transform: translateY(-45px) scale(1) translateX(3px); opacity: 0; }
}

.cooking-vessel { animation: gentle-float 4s ease-in-out infinite; }
@keyframes gentle-float {
  0%, 100% { transform: translateY(0); }
  50% { transform: translateY(-4px); }
}

.stirring-spoon { transform-origin: 70px 50px; animation: stir 2s ease-in-out infinite; }
@keyframes stir {
  0%, 100% { transform: rotate(-15deg); }
  50% { transform: rotate(15deg); }
}

.frying-bubble { animation: fry-pop 1.2s ease-out infinite; }
@keyframes fry-pop {
  0%, 100% { transform: translateY(0) scale(1); opacity: 0.5; }
  50% { transform: translateY(-6px) scale(1.2); opacity: 0.8; }
}

.chopping-knife { transform-origin: 70px 28px; animation: chop 0.7s ease-in-out infinite; }
@keyframes chop {
  0%, 100% { transform: rotate(0deg) translateY(0); }
  50% { transform: rotate(-12deg) translateY(-8px); }
}

.boiling-bubble { animation: boil-rise 1.4s ease-out infinite; }
@keyframes boil-rise {
  0% { transform: translateY(0) scale(1); opacity: 0.6; }
  100% { transform: translateY(-12px) scale(0.6); opacity: 0; }
}

.saute-piece { animation: saute-jump 0.8s ease-in-out infinite; }
@keyframes saute-jump {
  0%, 100% { transform: translateY(0) rotate(0deg); }
  50% { transform: translateY(-10px) rotate(10deg); }
}

.simmer-bubble { animation: simmer-pulse 2.5s ease-in-out infinite; }
@keyframes simmer-pulse {
  0%, 100% { transform: scale(1); opacity: 0.3; }
  50% { transform: scale(1.3); opacity: 0.6; }
}

.flying-ingredient { animation: fly-in 4s ease-in-out infinite; filter: drop-shadow(0 1px 2px rgba(0, 0, 0, 0.08)); }
@keyframes fly-in {
  0%, 100% { transform: translate(0, 0) rotate(0deg) scale(0.9); opacity: 0; }
  15% { opacity: 0.9; }
  50% { transform: translate(12px, -20px) rotate(120deg) scale(1); opacity: 0.9; }
  85% { opacity: 0.9; }
}

.action-verb { display: inline-block; }
.fade-verb-enter-active, .fade-verb-leave-active { transition: all 0.4s ease; }
.fade-verb-enter-from { opacity: 0; transform: translateY(8px); }
.fade-verb-leave-to { opacity: 0; transform: translateY(-8px); }

.fade-desc-enter-active, .fade-desc-leave-active { transition: all 0.3s ease; }
.fade-desc-enter-from { opacity: 0; }
.fade-desc-leave-to { opacity: 0; }

.cooking-text {
  background: linear-gradient(90deg, var(--color-brandp-500) 0%, var(--color-brandp-400) 25%, var(--color-brandp-300) 50%, var(--color-brandp-400) 75%, var(--color-brandp-500) 100%);
...
:root.dark .cooking-text {
  background: linear-gradient(90deg, var(--color-brandp-600) 0%, var(--color-brandp-500) 25%, var(--color-brandp-400) 50%, var(--color-brandp-500) 75%, var(--color-brandp-600) 100%);
  background-size: 200% auto;
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
...
.dot { animation: dot-pulse 1.8s ease-in-out infinite; }
